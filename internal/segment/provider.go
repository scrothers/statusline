...
// routed through: AWS Bedrock, Google Vertex, Microsoft Foundry/Azure,
// Cloudflare AI Gateway, DigitalOcean's inference endpoint, a generic
// third-party router (OpenRouter and similar), or an unrecognized
// corporate/self-hosted gateway. Only the glyph varies by provider — the
// color stays the theme's flat IdentityAccent, matching how repoSegment
// colors its host-branded icon — so this reads as a small supplementary
// badge, not a second accent system competing with the model segment's
// per-family colors.
//
...
// Azure/Foundry, Cloudflare, DigitalOcean, or a bare corporate-relayed id,
// none of which carry any distinguishing shape in the id itself.
// modelid.DetectProvider (id-shape heuristics) is only the last-resort
// fallback when neither of those set anything. A plain first-party id with
// no gateway involved has nothing to detect at any tier, so the segment is
...
func providerIconKey(p modelid.Provider) string {
	...
	case modelid.ProviderCloudflare:
		return theme.IconProviderCloudflare
	case modelid.ProviderDigitalOcean:
		return theme.IconProviderDigitalOcean
	default:
		return ""
	}
}
